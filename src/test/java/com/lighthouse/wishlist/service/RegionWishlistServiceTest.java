...
class RegionWishlistServiceTest {
    ...
    void setup() {
        ...
        service = new RegionWishlistService(mapper, regionCodeService);
    }

    @Test
    void saveOrUpdateWishlist_updateSuccess() {
        LikeRegion existing = new LikeRegion();
        when(regionCodeService.findRegionByRegionCd("1120010100").getLocallowNm()).thenReturn("강남동");
    ...
    void saveOrUpdateWishlist_invalidRegionCd() {
        when(regionCodeService.findRegionByRegionCd("1120010100").getLocallowNm()).thenReturn(null);
