...
class EstateWishlistMapperTest {
    ...
    void saveLikeEstate() {
        ...
        likeEstate.setEstateId(999L);
        likeEstate.setMemberId(34L);
        likeEstate.setIsLike(1);

        mapper.saveLikeEstate(likeEstate);

        LikeEstate result = mapper.findByMemberIdAndEstateId(34L, 999L);
    ...
    void updateLikeEstate() {
        ...
        LikeEstate updated = mapper.findByMemberIdAndEstateId(34L, 999L);
    ...
    @Test
    void findByMemberIdAndEstateId_whenExists_thenReturnData() {
        LikeEstate likeEstate = new LikeEstate();
        likeEstate.setEstateId(10L);
        likeEstate.setMemberId(34L);
        likeEstate.setIsLike(1);
        mapper.saveLikeEstate(likeEstate);

        LikeEstate result = mapper.findByMemberIdAndEstateId(34L, 10L);
        assertNotNull(result);
        assertEquals(1, result.getIsLike());
    ...
    @Test
    void findByMemberIdAndEstateId_whenNotExists_thenReturnNull() {
        LikeEstate result = mapper.findByMemberIdAndEstateId(999L, 888L);
        assertNull(result);
    ...
    void findEstateIdsByMemberId_returnsIds() {
        ...
        List<EstateWishlistResponseDTO> estateIds = mapper.findEstatesByMemberId(memberId);

        assertNotNull(estateIds);
        assertFalse(estateIds.isEmpty());

        log.info("Found estate IDs for memberId {}: {}", memberId, estateIds.get(0).getBuildingName());
    }
}